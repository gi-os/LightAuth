...
/** RFC 6238 TOTP over RFC 4226 HOTP truncation. Pure JVM, so it unit-tests directly. */
...
/** Lives outside the composable so it can be tested without an Android runtime. */
fun formatExpiryCountdown(totalSeconds: Int): String {
    val minutes = totalSeconds / 60
    val seconds = totalSeconds % 60
    return "$minutes:${seconds.toString().padStart(2, '0')}"
}

/** Spaced in the middle so a six- or eight-digit code can be read off in one glance. */
fun groupCodeDigits(code: String): String = when {
    code.length % 2 == 0 && code.length >= 6 ->
